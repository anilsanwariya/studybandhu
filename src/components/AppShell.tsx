        ...
        <div className="glass-strong rounded-3xl p-5 flex-1 flex lg:flex-col gap-4 items-center lg:items-stretch">
          <div className="flex items-center gap-2 lg:mb-4">
            ...
            <div className="hidden lg:block">
              <div className="font-semibold leading-none">StudyBandhu</div>
          ...
          <div className="hidden lg:flex flex-col gap-2 mt-auto">
            ...
            {user && (
              <Link
                to="/profile"
                className={cn(
                  "glass rounded-2xl px-3 py-2 flex items-center gap-2.5 transition-all",
                  pathname === "/profile" ? "bg-white/80 border-primary/40 ring-1 ring-primary/30" : "hover:bg-white/60"
                )}
              >
                <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-[var(--lavender)] to-[var(--sky)] flex items-center justify-center text-xs font-bold shrink-0">
                  {user.avatarUrl ? <img src={user.avatarUrl} className="h-full w-full rounded-xl object-cover" alt="" /> : initials(user.name)}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="text-xs font-semibold truncate">{user.name}</div>
                  <div className="text-[10px] text-muted-foreground truncate">
                    {user.targetExam ? `🎯 ${user.targetExam}` : "Profile"}
                  </div>
                </div>
              </Link>
            )}
          ...
          <div className="lg:hidden flex items-center gap-2 ml-auto">
            ...
            {user && (
              <Link to="/profile" className="glass rounded-full h-8 w-8 flex items-center justify-center text-xs font-bold">
                {initials(user.name)}
              </Link>
            )}
          </div>
