...
export function MorningIntent() {
  ...
  const scopedTree = useMemo(
    () => scopeTree(tree, subjectFilter, chapterFilter),
  ...
  const dueTree = useMemo(
    () => filterTree(scopedTree, (n) => !!n.dueToday && n.status !== "unread"),
    [scopedTree],
  );
  ...
  return (
    <AppShell>
      ...
      <div className="glass rounded-3xl px-4 py-3 flex items-start gap-3 mb-5">
        ...
        <p className="text-sm font-medium italic text-foreground/80 leading-relaxed break-words">
          "{quote}"
        </p>
      </div>
            ...
            {bucketNodes.length === 0 ? (
              ...
              bucketNodes.map((n) => (
                <div key={n.id} className="glass rounded-2xl px-3 py-2.5 flex items-start gap-3 min-w-0">
                  <div className="pt-1"><StatusDot status={n.status} /></div>
...
function BankNode({
  ...
  return (
    <div>
      <div
        className={cn(
          "flex items-start gap-1.5 rounded-2xl px-2 py-2 transition-all min-w-0",
